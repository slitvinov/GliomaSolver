...
void MRAG_BBInfoCreator<WaveletsType, BlockType>::_computeEasyGhosts_Synthesis(
  ...
  Matrix3D<char, true, std::allocator> matNeeded(size_iteration_nspace[0],
                                                 size_iteration_nspace[1],
                                                 size_iteration_nspace[2]);
  Matrix3D<int, true, std::allocator> matKey(size_iteration_nspace[0],
                                             size_iteration_nspace[1],
                                             size_iteration_nspace[2]);
  Matrix3D<int, true, std::allocator> matDest(
    ...
    for (i[2] = s[2]; i[2] < e[2]; i[2]++)
      for (i[1] = s[1]; i[1] < e[1]; i[1]++)
        for (i[0] = s[0]; i[0] < e[0]; i[0]++) {
          ...
          BastardGhost *bastard = allocator<BastardGhost>().allocate(1);
          allocator<BastardGhost>().construct(bastard,
                                              BastardGhost(b.level, i));
...
void MRAG_BBInfoCreator<WaveletsType, BlockType>::_resolveBastardGhosts(
  ...
  // old front with the new front, goto 2. 8.	collect the shit 9.	free the
  // shit
