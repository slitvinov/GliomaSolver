...
protected:
  ...
  template <typename T> inline std::allocator<T> allocator() const {
  ...
  void prepare(const BlockCollection<BlockType> &collection,
    ...
    if (m_cacheBlock == NULL ||
        ...
        m_cacheBlock->getSize()[1] !=
            BlockType::sizeX + m_stencilEnd[1] - m_stencilStart[1] - 1 ||
        m_cacheBlock->getSize()[2] !=
            BlockType::sizeX + m_stencilEnd[2] - m_stencilStart[2] - 1) {

      if (m_cacheBlock != NULL)
        _release(m_cacheBlock);

      m_cacheBlock =
          allocator<Matrix3D<ElementType, true, std::allocator>>().allocate(1);

      allocator<Matrix3D<ElementType, true, std::allocator>>().construct(
          m_cacheBlock, Matrix3D<ElementType, true, std::allocator>());
