...
struct I4 {
  ...
  bool operator<(const I4 &i) const {
    return (l < i.l || l == i.l && x < i.x || l == i.l && x == i.x && y < i.y ||
...
#endif
      inline void
      pos(T p[3], int ix, int iy, int iz) const {
    p[0] = origin[0] + h[0] * ix;
    p[1] = origin[1] + h[1] * iy;
    p[2] = origin[2] + h[2] * iz;
  }
...
struct UniformPartition {
  ...
  void setup(const Integer s[3], const Integer e[3], const int nThreads[3],
    ...
    if (vMaxWorkPerThread != NULL) {
      work_per_thread[0] = std::min(vMaxWorkPerThread[0],
                                    std::max(1, total_work[0] / nThreads[0]));
      work_per_thread[1] = std::min(vMaxWorkPerThread[1],
                                    std::max(1, total_work[1] / nThreads[1]));
      work_per_thread[2] = std::min(vMaxWorkPerThread[2],
                                    std::max(1, total_work[2] / nThreads[2]));
    } else {
  ...
  __device__
#endif
      bool
      findBlockWorkingArea(const int passID, int base_index[3],
                           int end_index[3]) {
    if (passID >= passes())
      return false;
    ...
    end_index[0] =
        start[0] + std::min(offset[0] + threads[0] * work_per_thread[0],
                            (int)total_work[0]);
    end_index[1] =
        start[1] + std::min(offset[1] + threads[1] * work_per_thread[1],
                            (int)total_work[1]);
    end_index[2] =
        start[2] + std::min(offset[2] + threads[2] * work_per_thread[2],
                            (int)total_work[2]);
...
#endif
      void
      findThreadIndex(int threadID, int output[3]) {
...
#endif
      void
      getCoveredInterval(int s[3], int e[3]) {
    s[0] = start[0];
    s[1] = start[1];
    s[2] = start[2];
  ...
  __device__
#endif
      bool
      findWorkingInterval(const int thread_index[3], const int passID, int s[3],
                          int e[3]) {
    if (passID >= passes())
      return false;
    ...
    e[0] =
        start[0] + std::min(offset[0] + work_per_thread[0], (int)total_work[0]);
    e[1] =
        start[1] + std::min(offset[1] + work_per_thread[1], (int)total_work[1]);
    e[2] =
        start[2] + std::min(offset[2] + work_per_thread[2], (int)total_work[2]);
