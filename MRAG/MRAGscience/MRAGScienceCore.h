...
int AutomaticRefinement(Grid &g, BlockFWT &fwt, const double dAbsoluteTolerance,
  ...
  do {
    ...
    if (bVerbose)
      printf("Blocks to FWT (levelset): %zu\n", vBlocksToFWT.size());
    ...
    for (int i = 0; i < vBlocksToFWT.size(); i++) {
      if (bVerbose)
        printf(
            "getOverAll_DetailMaxMag()=%e >= dAbsoluteTolerance=%e -----> %s\n",
            vReports[i].getOverAll_DetailMaxMag(), dAbsoluteTolerance,
            vReports[i].getOverAll_DetailMaxMag() >= dAbsoluteTolerance
                ? "to refine!"
                : "skept");
    ...
    if (bVerbose)
      printf("AutomaticRefinement:: Refined %zu, FWT skipped blocks:%d\n",
...
RefinementResult AutomaticRefinementForLevelsets(
    ...
    const int iMaxLoops = -1, double *dMaxDetailAlive = NULL,
    void (*fillGrid)(Grid &g) = NULL) {
