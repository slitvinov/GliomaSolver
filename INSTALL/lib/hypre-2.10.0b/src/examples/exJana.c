/*
   ...
   Description:    This example differs from the previous structured example
                ...
                K - phase field function - [0.025,0.5]^2 inner domain
                eps - smoothenitnhg width
                   in the unit square with
                   boundary condition u = U0. 
                    --------------------
 
                    modifcaiton of hypre example 4: 
                    div (-K grad u + B u) + C u = F
...
int main (int argc, char *argv[])
{
   ...
   if ((solver_id > 29) && (solver_id < 40))
   {
    ...
    if(myid == 0)
    {
        ...
        sprintf(filename, "%s", "exJana_u.dat");
        ...
        sprintf(filename, "%s", "exJana_rhs.dat");
        if ((file = fopen(filename, "w")) == NULL)
        {
            printf("Error: can't open output file %s\n", filename);
            MPI_Finalize();
...
//        free(values);
        ...
        sprintf(filename, "%s", "exJana_psi.dat");
        if ((file = fopen(filename, "w")) == NULL)
        {
            printf("Error: can't open output file %s\n", filename);
            MPI_Finalize();
   ...
   /* Save the solution for GLVis visualization, see vis/glvis-exJana.sh */
   if (vis)
   {
      FILE *file;
      char filename[255];
      ...
      sprintf(filename, "%s.%06d", "vis/exJana.sol", myid);
      if ((file = fopen(filename, "w")) == NULL)
      {
         printf("Error: can't open output file %s\n", filename);
         MPI_Finalize();
      ...
      if (myid == 0)
         GLVis_PrintGlobalSquareMesh("vis/exJana.mesh", N*n-1);
   }
